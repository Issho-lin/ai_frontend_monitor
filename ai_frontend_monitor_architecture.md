...
| 模块 | 推荐选型 | 选择理由 |
| --- | --- | --- |
| Web SDK | TypeScript + Web Performance API + web-vitals | 轻量、可控、适合浏览器真实用户监控 |
| 接入服务 | Go / Node.js Fastify | 高吞吐、易水平扩展 |
| 消息队列 | Kafka / Redpanda | 支持高吞吐事件流和回放 |
| 明细分析库 | ClickHouse | 高吞吐列式分析，适合事件明细和多维聚合 |
| 缓存 | Redis | 告警状态、采样配置、热点查询缓存 |
| 后端 API | Go / NestJS | 受控查询、项目权限、工作流接口 |
| 前端控制台 | React + TypeScript + Vite | 管理台以 SPA 为主，构建轻量 |
| 图表 | Apache ECharts | 性能趋势、分布图、瀑布图生态成熟 |
| Agent 编排 | LangGraph 或 OpenAI Agents SDK | 便于工具调用、步骤编排、状态管理和可观测 |
| 对象存储 | S3 / MinIO | 存储原始归档、报告附件、sourcemap |
| 部署 | Kubernetes + Helm | 服务可水平扩展，适合生产运行 |
...
### 11.1 `create_task_draft`：治理任务草稿接口
...
### 11.2 `start_verification`：修复验证接口
